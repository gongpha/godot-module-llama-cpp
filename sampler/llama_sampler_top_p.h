...
class LlamaSamplerTopP : public LlamaSampler {
	GDCLASS(LlamaSamplerTopP, LlamaSampler);
...
protected:
	static void _bind_methods() {
		ClassDB::bind_method(D_METHOD("set_p", "p"), &LlamaSamplerTopP::set_p);
		ClassDB::bind_method(D_METHOD("get_p"), &LlamaSamplerTopP::get_p);
		ClassDB::bind_method(D_METHOD("set_min_keep", "min_keep"), &LlamaSamplerTopP::set_min_keep);
	...
	struct llama_sampler* _make_sampler(const llama_vocab *vocab) override { return llama_sampler_init_top_p(p, (size_t)min_keep); }
